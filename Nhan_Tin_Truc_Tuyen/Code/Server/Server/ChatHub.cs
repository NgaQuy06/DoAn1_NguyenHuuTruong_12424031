    ...
    public class ChatHub : Hub // Trung tâm kết nối
    {
        ...
        public async Task GuiTNRieng(int maTK, string ngGui, int maCTC, string ngNhan, string mess)
        {
            try
            {
                ...
                // gửi cho người nhận
                await Clients.User(ngNhan).SendAsync("NhanTNRieng", new object[] { new TNRieng { MaCTC = maCTC, NgGui = ngGui, NoiDung = mess } });

                // gửi lại cho chính mình (để hiển thị)
                await Clients.Caller.SendAsync("NhanTNRieng", new object[] { new TNRieng { MaCTC = maCTC, NgGui = ngGui, NoiDung = mess } });
            }
            catch (Exception ex)
            {
